...
define('generatorApp', ['angular', 'ngRoute', 'ngMaterial', 'ngAria', 'ngAnimate', 'ngMessages', 'ajv'],
    function(angular, ngRoute, ngMaterial, ngAria, ngAnimate, ngMessages, ajv){

    let generatorApp = angular.module('generatorApp',
    ...
    generatorApp.controller('documenterController', ['$scope','$location','$http','$templateCache','SchemaLoader',
        function($scope, $location, $http, $templateCache, SchemaLoader) {
            $templateCache.removeAll();

            let test = new ajv({schemaId: 'auto'});
            ...
            schema_loader.load_schema(json_schema.target, 0, null).then(
                function(){
                    ...
                    console.log(json_schema.loaded_specs);
            ...
            json_schema.displayItem = function(itemName){
            ...
            json_schema.display_menu = function(){
            ...
            json_schema.create_href = function(context, fieldName) {
    ...
    /* TEMPLATES */
    generatorApp.directive('schemaLoader', function() {
    let base_path = window.location.origin;
    return {
        restrict: 'A',
        templateUrl: base_path + '/json_documenter/include/schema.html',
        scope: {
            schemaLoader: '=',
            schemaName: '=',
            container: "="
    ...
    generatorApp.directive('schemaFields', function(){
    let base_path = window.location.origin;
    return{
        restrict: 'A',
        templateUrl:  base_path + '/json_documenter/include/fields.html',
        scope: {
            schemaFields: '=',
            parentKey: '=',
            contextValues: '=',
    ...
    generatorApp.directive('fieldType', function(){
    let base_path = window.location.origin;
    return{
        restrict: 'A',
        templateUrl:  base_path + '/json_documenter/include/field_type.html',
        scope: {
            fieldType: '=',
        },
    ...
    generatorApp.directive('buttonLink', function(){
        let base_path = window.location.origin;
        return{
            restrict: 'A',
            templateUrl:  base_path + '/json_documenter/include/link_button.html',
            scope: {
                buttonLink: '=',
                isRequired: "="
            },
    ...
    /* FILTERS */
    generatorApp.filter('removeExtraStr', function() {
    ...
    generatorApp.filter('ellipsisFilter', function() {
    return function(input) {
        if (input.length>12) {
            console.log(input);
    ...
    generatorApp.filter('typeOf', function() {
        return function (obj) {
    ...
    generatorApp.service('ngCopy', ['$window', function ($window) {
    ...
    generatorApp.directive('ngClickCopy', ['ngCopy', function (ngCopy) {
        return {
            restrict: 'A',
    ...
    /* bootstrap tooltip */
    generatorApp.directive('tooltip', function(){
        return {
            restrict: 'A',
            scope: {
              fieldName: '='
